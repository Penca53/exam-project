...
std::unique_ptr<ng::Scene> MakeDefaultScene(ng::App* app) {
  ...
  auto& player = scene->MakeChild<Player>(&tilemap_walls, &camera);
  camera.MakeChild<FollowPlayer>(&player);

  auto& spawner_0 = scene->MakeChild<EnemySpawner>(
      &player, &tilemap_walls, EnemySpawner::EnemyType::kGhost);
  spawner_0.SetLocalPosition({-200, 0});

  auto& spawner_1 = scene->MakeChild<EnemySpawner>(
      &player, &tilemap_walls, EnemySpawner::EnemyType::kShootingGhost);
  spawner_1.SetLocalPosition({200, 0});
  return scene;
}
