...
Ghost::Ghost(ng::App* app, Player* player, const ng::Tilemap* tilemap)
    : ng::Node(app),
      player_(player),
      tilemap_(tilemap),
      sprite_(GetApp()->GetResourceManager().LoadTexture(
          "Sprites/Ghost/ghost.png")),
      death_sound_(GetApp()->GetResourceManager().LoadSoundBuffer("Pop_1.wav")),
      gen_(rd_()),
      health_pickup_drop_(0, 100) {
  SetName("Ghost");
...
void Ghost::TakeDamage(float damage) {
  ...
  if (health_ <= 0) {
    ...
    death_sound_.play();
    player_->AddScore(50);
    if (health_pickup_drop_(gen_) < 25) {
      auto& health_pickup = GetScene()->MakeChild<HealthPickup>();
      health_pickup.SetLocalPosition(GetGlobalTransform().getPosition());
    }
  }
}
