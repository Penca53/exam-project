...
Player::Player(ng::App* app, const ng::Tilemap* tilemap,
               const ng::Camera* camera, UIManager* ui_manager,
    ...
    : ng::Node(app),
      ...
      hit_sound_(GetApp()->GetResourceManager().LoadSoundBuffer("Hit_2.wav")),
      pickup_sound_(
          GetApp()->GetResourceManager().LoadSoundBuffer("Powerup_4.wav")) {
  SetName("Player");
  head_.setOrigin({4, 2});
  ...
  player_weapon_holder_ =
      &MakeChild<PlayerWeaponHolder>(this, camera, tilemap_, game_manager_);
  collider_ = &MakeChild<ng::RectangleCollider>(sf::Vector2f(16, 16));

  ui_manager_->UpdateHealth(health_);
...
void Player::TakeDamage(float damage) {
  if (is_dead_ || game_manager_->GetState() == GameManager::State::WON) {
    return;
  }
...
void Player::Update() {  // NOLINT
  ...
  if (large_bullet_timer_ > 0) {
    --large_bullet_timer_;
    if (large_bullet_timer_ == 0) {
      player_weapon_holder_->SetIsLargeBulletsEnabled(false);
    }
  }
  ...
  for (const auto* other : others) {
    if (other->GetParent()->GetName() == "HealthPickup") {
      auto* health_pickup = dynamic_cast<HealthPickup*>(other->GetParent());
      health_pickup->Pickup(this);
      pickup_sound_.play();
    } else if (other->GetParent()->GetName() == "LargeBulletPickup") {
      auto* large_bullet_pickup =
          dynamic_cast<LargeBulletPickup*>(other->GetParent());
      large_bullet_pickup->Pickup();
      large_bullet_timer_ = 60 * 8;
      player_weapon_holder_->SetIsLargeBulletsEnabled(true);
      pickup_sound_.play();
    }
  }
}
