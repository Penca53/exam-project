...
ShootingGhost::ShootingGhost(ng::App* app, Player* player,
    ...
    : ng::Node(app),
      ...
      gen_(rd_()),
      large_bullet_pickup_drop_(0, 100) {
...
void ShootingGhost::TakeDamage(float damage) {
  ...
  if (health_ <= 0) {
    ...
    if (large_bullet_pickup_drop_(gen_) < 25) {
      auto& large_bullet_pickup = GetScene()->MakeChild<LargeBulletPickup>();
      large_bullet_pickup.SetLocalPosition(GetGlobalTransform().getPosition());
    }
  }
}
